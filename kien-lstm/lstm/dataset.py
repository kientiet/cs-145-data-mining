...
class CustomDataset(Dataset):
    ...
    def __dataframe_to_time_series(self, dataset: pd.DataFrame) -> np.ndarray:
        ...
        self.new_dataset = dataset.copy(deep=True)
    ...
    def inverse_confirmed(self, confirmed: np.ndarray) -> np.ndarray:
        min_value = self.scaler.min_[-2]
        return (confirmed - min_value) / self.scaler.scale_[-2]

    def inverse_deaths(self, deaths: np.ndarray) -> np.ndarray:
        min_value = self.scaler.min_[-1]
        return (deaths - min_value) / self.scaler.scale_[-1]
